  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8">
      ...
      <div className="rounded-2xl border bg-white p-6 space-y-4">
        <p className="text-xs font-medium text-slate uppercase tracking-wide">ROI estimado</p>

        <div className="grid grid-cols-3 gap-4">
          <div>
            <p className="text-xs text-slate">Últimos 7 días</p>
            <p className="text-2xl font-bold text-ink tabular-nums mt-0.5">
              S/ {Math.round(value.roi.roi7d).toLocaleString('es-PE')}
            </p>
          </div>
          <div>
            <p className="text-xs text-slate">Este mes</p>
            <p className="text-2xl font-bold text-ink tabular-nums mt-0.5">
              S/ {Math.round(value.roi.roi30d).toLocaleString('es-PE')}
            </p>
          </div>
          <div>
            <p className="text-xs text-slate">Acumulado histórico</p>
        ...
        <div className="border-t pt-4">
          ...
          <div className="mt-2 h-2 bg-[#F3F6F9] rounded-full overflow-hidden">
            <div
          ...
          {value.message && (
            <p className="text-sm text-slate mt-2 font-medium">{value.message}</p>
          )}
        </div>
      </div>
      ...
      <div className="rounded-2xl border bg-white p-6 space-y-4">
        <div className="flex items-center justify-between">
          <p className="text-xs font-medium text-slate uppercase tracking-wide">Value score</p>
          <span className={`text-2xl font-bold tabular-nums ${value.valueScore.total >= 70 ? 'text-lima-600' : value.valueScore.total >= 40 ? 'text-amber-600' : 'text-red-500'}`}>
            {value.valueScore.total}<span className="text-sm font-normal text-slate">/100</span>
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {[
            ...
            <div key={d.label} className="bg-mist rounded-lg p-3">
              <p className="text-[11px] text-slate">{d.label}</p>
              <div className="flex items-end gap-1 mt-1">
                <span className={`text-lg font-bold tabular-nums ${d.val < d.max * 0.4 ? 'text-amber-500' : 'text-ink'}`}>
                  {d.val}
                </span>
                <span className="text-xs text-fog mb-0.5">/{d.max}</span>
              </div>
              <div className="mt-1 h-1 bg-fog rounded-full overflow-hidden">
                <div
        ...
        {value.topAction !== 'Sin actividad registrada' && (
          <p className="text-xs text-slate">
            Principal generador de valor este mes: <span className="text-slate font-medium">{value.topAction}</span>
          </p>
        )}
      </div>
...
function ROIMultiplierBadge({ multiplier }: { multiplier: number }) {
  if (multiplier >= 2)   return <span className="text-xs font-bold text-lima-700 bg-lima-100 px-2 py-0.5 rounded-full">Ya recuperaste {multiplier.toFixed(1)}× tu plan</span>
  ...
  return <span className="text-xs text-slate bg-[#F3F6F9] px-2 py-0.5 rounded-full">Potencial por activar</span>
}
